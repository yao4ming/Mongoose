...
function recurseAccordion(obj, id) {
	...
	if (typeof Object.values(obj)[0] === "string") {
      ...
      for (var i in obj) {
         ...
         headerHTML += "<div class=\"accordion-header\"> <a id=\"link\" href=\"#\"><span>"  + obj[i] + "</span></a> </div>";
...
Accordion.prototype = {
   ...
   onFormReady: function() {
     ...
     $('#' + accordionId + " > .accordion-pane > .accordion-header > a").click(function() {
         console.log(this);
         ctx.setValue(this.children[0].innerHTML);
     });

     $('#' + accordionId).parent().css("overflow", "auto");
   ...
   onValueChanged: function(newVal) {
     ...
     if (newVal) {
         ...
         var accordionJSON = JSON.parse(newVal);
